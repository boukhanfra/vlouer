...
{
    var id_country = $('#fos_user_profile_form_trader_country').val();
    $.ajax({
        ...
        success: function (json) { // quand la réponse de la requete arrive
            ...
            $('#fos_user_profile_form_trader_city').html('');
            $.each(json, function (index, value) { // et  boucle sur la réponse contenu dans la variable passé à la function du success "json"
                $('#fos_user_profile_form_trader_city').append('<option value="' + value.id + '">' + value.nom + '</option>');
            });
        }
    });
}